  ...
  const handleDrivingChoice = (choice: string, phase: string) => {
      ...
      setGameState(prev => ({
        ...
        consecutiveRouteStreak:
          prev.consecutiveRouteStreak?.type === routeChoice
            ? {
                type: routeChoice,
                count: prev.consecutiveRouteStreak.count + 1,
              }
  ...
  const completeRide = useCallback(() => {
    ...
    if (Math.random() < backstoryChance) {
      backstoryUnlocked = {
        ...
        backstory:
          passenger.backstoryDetails ||
          `${passenger.name} shares a mysterious secret from their past...`,
      };
    }
