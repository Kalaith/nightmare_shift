...
function printHeader() {
  ...
  console.log(
    "║" + " ".repeat(28) + "🚗 NIGHTMARE SHIFT" + " ".repeat(28) + "║",
  );
  console.log(
    "║" + " ".repeat(23) + "Automated Balance Testing" + " ".repeat(23) + "║",
  );
  console.log(
    "╚════════════════════════════════════════════════════════════════════════════╝" +
      colors.reset +
...
console.log(
  `   Strategies: ${colors.yellow}${STRATEGIES.length}${colors.reset}`,
);
console.log(
...
let completedRuns = 0;
const strategyProgress: Record<Strategy, { runs: number; successes: number }> =
  {} as any;
...
for (let i = 0; i < TOTAL_RUNS; i++) {
  ...
  if (!result.success) {
    ...
    failureReasons[strategy][reason] =
      (failureReasons[strategy][reason] || 0) + 1;
  }
  ...
  if (completedRuns % 6 === 0 || completedRuns === TOTAL_RUNS) {
    ...
    STRATEGIES.forEach((s) =>
      printStrategyStatus(
        s,
        strategyProgress[s].runs,
        strategyProgress[s].successes,
      ),
    );
  }
}
...
STRATEGIES.forEach((strategy) => {
  ...
  strategyStats[strategy] = {
    ...
    avgEarnings: Math.round(
      stratResults.reduce((sum, r) => sum + r.finalEarnings, 0) /
        stratResults.length,
    ),
    avgRides:
      Math.round(
        (stratResults.reduce((sum, r) => sum + r.ridesCompleted, 0) /
          stratResults.length) *
          10,
      ) / 10,
  };
...
const stats = {
  ...
  averageEarnings: Math.round(
    results.reduce((sum, r) => sum + r.finalEarnings, 0) / TOTAL_RUNS,
  ),
  averageRides:
    Math.round(
      (results.reduce((sum, r) => sum + r.ridesCompleted, 0) / TOTAL_RUNS) * 10,
    ) / 10,
