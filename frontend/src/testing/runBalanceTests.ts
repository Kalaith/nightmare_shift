...
const TOTAL_RUNS = 120; // Divisible by 6 strategies
const STRATEGIES: Strategy[] = ['shortcut_spam', 'balanced', 'strategic', 'perfect', 'scenic_only', 'random'];
...
function printProgressBar(current: number, total: number, width: number = 50): string {
    ...
    const bar = colors.bgGreen + ' '.repeat(filled) + colors.reset + colors.dim + '░'.repeat(width - filled) + colors.reset;
...
function printStrategyStatus(strategy: Strategy, runs: number, successes: number) {
    ...
    console.log(`  ${color}${icon}${colors.reset} ${strategy.replace('_', ' ').toUpperCase().padEnd(20)} ${runs} runs, ${rate}% success`);
}
...
console.log(`   Runs per strategy: ${colors.cyan}${TOTAL_RUNS / STRATEGIES.length}${colors.reset}\n`);
...
for (let i = 0; i < TOTAL_RUNS; i++) {
    ...
    if (completedRuns % 6 === 0 || completedRuns === TOTAL_RUNS) {
        ...
        STRATEGIES.forEach(s => printStrategyStatus(s, strategyProgress[s].runs, strategyProgress[s].successes));
    }
}
...
console.log(`   Average: ${colors.cyan}${(parseFloat(elapsedTime) / TOTAL_RUNS * 1000).toFixed(0)}ms${colors.reset} per simulation\n`);

const strategyStats: Record<Strategy, { runs: number; successes: number; avgEarnings: number; avgRides: number }> = {} as any;
STRATEGIES.forEach(strategy => {
    ...
    strategyStats[strategy] = {
        ...
        avgEarnings: Math.round(stratResults.reduce((sum, r) => sum + r.finalEarnings, 0) / stratResults.length),
        avgRides: Math.round((stratResults.reduce((sum, r) => sum + r.ridesCompleted, 0) / stratResults.length) * 10) / 10
    };
});
...
const stats = {
    ...
    averageRides: Math.round((results.reduce((sum, r) => sum + r.ridesCompleted, 0) / TOTAL_RUNS) * 10) / 10,
    strategyResults: strategyStats
};

GameSimulator.printReport(stats);
console.log(colors.dim + '💾 Results saved to memory' + colors.reset + '\n');
