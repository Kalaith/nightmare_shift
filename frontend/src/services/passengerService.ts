...
export class PassengerService {
      ...
      () => {
        ...
        if (availablePassengers.length === 0) {
          ...
          if (!fallbackPassenger) {
            throw ErrorHandling.serviceError(
              "PassengerService",
        ...
        const selectedPassenger = this.selectByRarityUnsafe(
          availablePassengers,
          difficultyLevel,
        );
        if (!selectedPassenger) {
          throw ErrorHandling.serviceError(
            "PassengerService",
            "selectRandomPassenger",
            "Failed to select passenger by rarity",
          );
        }
  ...
  static getAdjustedRarityWeights(
    ...
    switch (difficultyLevel) {
      ...
      case 2:
        return {
          ...baseWeights,
          rare: baseWeights.rare * 1.5,
          legendary: baseWeights.legendary * 2,
        };
      case 3:
        return {
          ...baseWeights,
          rare: baseWeights.rare * 2,
          legendary: baseWeights.legendary * 3,
        };
      case 4:
      default:
        return {
          ...baseWeights,
          rare: baseWeights.rare * 3,
          legendary: baseWeights.legendary * 5,
        };
    }
  }

  static shouldSpawnRelatedPassenger(
    _completedRides: CompletedRide[],
  ): GameResult<Passenger | null> {
    return ErrorHandling.wrap(
      () => {
        if (Math.random() > GAME_BALANCE.PROBABILITIES.RELATED_PASSENGER_SPAWN)
          return null;
  ...
  static calculateBackstoryChance(
    passengerId: number,
    ...
    return Math.max(
      repeatChance,
      Math.min(1, (firstChance + repeatChance) / 2),
    );
  }
  ...
  static selectWeatherAwarePassenger(
    ...
    return ErrorHandling.wrap(
      () => {
        ...
        if (availablePassengers.length === 0) {
          const fallbackResult = this.selectRandomPassenger(
            usedPassengers,
            difficultyLevel,
          );
  ...
  private static applyEnvironmentalWeights(
    ...
    return passengers.map((passenger) => {
      ...
      weight *= this.getSpecialBehaviorModifier(
        passenger,
        weather,
        timeOfDay,
        season,
  ...
  private static getWeatherModifier(
    ...
    const weatherPreferences: Record<
      number,
      Partial<Record<WeatherCondition["type"], number>>
    > = {
    ...
    if (weather.intensity === "heavy") {
      ...
      if (
        passenger.supernatural.includes("supernatural") ||
   ...
   */
  private static getTimeModifier(
    passenger: Passenger,
    timeOfDay: TimeOfDay,
  ): number {
    const passengerId = passenger.id;
    ...
    const timePreferences: Record<
      number,
      Partial<Record<TimeOfDay["phase"], number>>
    > = {
    ...
    if (
      passenger.supernatural !== "Living" &&
      passenger.supernatural !== "Human"
    ) {
  ...
  private static getSpecialBehaviorModifier(
    ...
    if (
      weather.type === "fog" &&
      (timeOfDay.phase === "night" || timeOfDay.phase === "latenight")
    ) {
   ...
   */
  private static getBaseRarityWeight(
    rarity: Passenger["rarity"],
    difficultyLevel: number,
  ): number {
    const adjustedWeights = this.getAdjustedRarityWeights(difficultyLevel);
  ...
  private static selectFromWeightedPool(
    ...
    const totalWeight = weightedPassengers.reduce(
