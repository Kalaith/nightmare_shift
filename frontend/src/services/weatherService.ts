...
export class WeatherService {
  ...
  static generateEnvironmentalHazards(
    ...
    return ErrorHandling.wrap(
      () => {
        ...
        const hazardChance = this.calculateHazardChance(
          weather,
          timeOfDay,
          season,
        );
        ...
        if (Math.random() < hazardChance) {
          ...
          const hazard = this.createEnvironmentalHazard(
            hazardType,
            weather,
            timeOfDay,
          );
   ...
   */
  static getWeatherTriggeredRules(
    weather: WeatherCondition,
    timeOfDay: TimeOfDay,
  ): number[] {
  ...
  static updateWeather(
    ...
    return ErrorHandling.wrap(
      () => {
        ...
        if (elapsed >= currentWeather.duration * 60000) {
  ...
  private static getSeasonalWeatherTypes(
    season: Season,
  ...
  private static getRandomIntensity(
    type: WeatherCondition["type"],
  ...
  private static getWeatherEffects(
    ...
    const intensityMultiplier =
      intensity === "light" ? 1 : intensity === "moderate" ? 1.5 : 2;
    ...
    switch (type) {
      ...
      case "fog":
        effects.push(
          {
            type: "visibility_reduction",
            value: 20 * intensityMultiplier,
            description: "Poor visibility",
          },
          {
            type: "time_delay",
            value: 15 * intensityMultiplier,
            description: "Slower driving",
          },
        );
        break;
      case "snow":
        effects.push(
          {
            type: "visibility_reduction",
            value: 15 * intensityMultiplier,
            description: "Snow obscures view",
          },
          {
            type: "fuel_consumption",
            value: 10 * intensityMultiplier,
            description: "Cold weather fuel usage",
          },
          {
            type: "time_delay",
            value: 20 * intensityMultiplier,
            description: "Careful driving required",
          },
        );
        break;
      case "thunderstorm":
        effects.push(
          {
            type: "visibility_reduction",
            value: 25 * intensityMultiplier,
            description: "Heavy rain and darkness",
          },
          {
            type: "supernatural_attraction",
            value: 30 * intensityMultiplier,
            description: "Supernatural activity increases",
          },
          {
            type: "passenger_behavior",
            value: 20 * intensityMultiplier,
            description: "Passengers more agitated",
          },
        );
        break;
      case "wind":
        effects.push({
          type: "fuel_consumption",
          value: 8 * intensityMultiplier,
          description: "Fighting headwinds",
        });
        if (intensity === "heavy") {
          effects.push({
            type: "route_blockage",
            value: 20,
            description: "Some routes blocked by debris",
          });
        }
        break;
    }
  ...
  private static calculateVisibility(
    ...
    switch (type) {
      case "fog":
        baseVisibility =
          intensity === "light" ? 60 : intensity === "moderate" ? 30 : 10;
        break;
      case "rain":
      case "thunderstorm":
        baseVisibility =
          intensity === "light" ? 80 : intensity === "moderate" ? 60 : 40;
        break;
      case "snow":
        baseVisibility =
          intensity === "light" ? 70 : intensity === "moderate" ? 50 : 25;
        break;
      default:
        baseVisibility = 100;
    }
  ...
  private static getWeatherDescription(
    ...
    const descriptions = {
      clear: {
        light: "Clear skies with good visibility",
        moderate: "Clear skies with good visibility",
        heavy: "Clear skies with good visibility",
      },
      rain: {
        light: "Light drizzle dampens the streets",
        moderate: "Steady rain creates puddles and reflections",
        heavy: "Heavy downpour reduces visibility significantly",
      },
      fog: {
        light: "Thin fog creates an eerie atmosphere",
        moderate: "Dense fog obscures distant objects",
        heavy: "Thick fog makes driving treacherous",
      },
      snow: {
        light: "Light snowfall dusts the ground",
        moderate: "Steady snow accumulates on roads",
        heavy: "Heavy snowstorm creates whiteout conditions",
      },
      thunderstorm: {
        light: "Distant thunder rumbles ominously",
        moderate: "Lightning illuminates the dark clouds",
        heavy: "Violent thunderstorm rages overhead",
      },
      wind: {
        light: "Gentle breeze stirs the air",
        moderate: "Strong winds rock the vehicle",
        heavy: "Powerful gusts threaten to push cars off course",
      },
    };
  ...
  private static getSeasonalPassengerModifiers(
    type: Season["type"],
  ): Season["passengerModifiers"] {
    const modifiers = {
      spring: {
  ...
  private static selectHazardType(
    ...
    const types: EnvironmentalHazard["type"][] = [
      "construction",
      "accident",
      "road_closure",
    ];
    ...
    if (
      ...
      timeOfDay.supernaturalActivity > 70
    ) {
  ...
  private static createEnvironmentalHazard(
    ...
    const severity =
      Math.random() < 0.1 ? "extreme" : Math.random() < 0.3 ? "major" : "minor";
  ...
  private static createWeatherHazard(
    ...
    const hazardMap = {
      flooding: {
        ...
        description: "Flash flooding blocks roadway",
      },
      ice_roads: {
        type: "accident" as const,
        description: "Ice makes roads treacherous",
      },
      visibility: {
        type: "construction" as const,
        description: "Poor visibility causes delays",
      },
    };
  ...
  private static getHazardEffects(
    ...
    const severityMultiplier =
      severity === "minor" ? 1 : severity === "major" ? 2 : 3;
    ...
    return {
      routeBlocked:
        "routeBlocked" in effects ? effects.routeBlocked : undefined,
      timeDelay:
        "timeDelay" in effects && effects.timeDelay
          ? effects.timeDelay * severityMultiplier
          : undefined,
      fuelIncrease:
        "fuelIncrease" in effects && effects.fuelIncrease
          ? effects.fuelIncrease * severityMultiplier
          : undefined,
      riskIncrease:
        "riskIncrease" in effects && effects.riskIncrease
          ? effects.riskIncrease * severityMultiplier
          : undefined,
      forcedChoice:
        "forcedChoice" in effects ? effects.forcedChoice : undefined,
    };
  }

  private static getWeatherHazardEffects(
    hazardType: string,
  ): EnvironmentalHazard["effects"] {
    const effects = {
      flooding: {
        routeBlocked: ["normal", "shortcut"],
        timeDelay: 20,
        riskIncrease: 2,
      },
  ...
  private static getHazardDuration(
    ...
    const severityMultiplier =
      severity === "minor" ? 0.7 : severity === "major" ? 1 : 1.5;
