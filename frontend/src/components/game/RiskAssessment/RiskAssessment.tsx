...
export const RiskAssessment: React.FC<RiskAssessmentProps> = ({
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const newAnalysis = calculateRiskAnalysis(gameState, passenger, guidelines, detectedTells);
  ...
  const calculateRiskAnalysis = (
    ...
    const deceptionLikelihood = Math.min(
      0.9,
      deceptionLevel + (conflictingSignals ? 0.3 : 0) + (passengerStress > 0.7 ? 0.2 : 0)
    );

    // Calculate overall risk
    const tellsReliability =
      tells.length > 0 ? tells.reduce((sum, t) => sum + t.tell.reliability, 0) / tells.length : 0.5;
            ...
            <div className={styles.detailedAnalysis}>
              ...
              <div className={styles.analysisSection}>
                ...
                <div className={styles.tellAnalysis}>
                  ...
                  <span>
                    Obvious: {detectedTells.filter(t => t.tell.intensity === 'obvious').length}
                  </span>
