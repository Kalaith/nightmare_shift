...
export class PassengerVariationService {
    ...
    'eye_contact_lonely': [
      {
        ...
        triggerPhrases: ["Why won't you look", "acknowledge I'm here", "invisible", "looks away"],
    ...
    'speak_nervous_passenger': [
      {
        ...
        triggerPhrases: ["Talk to me", "silence is driving", "alone with my thoughts", "say something"],
    ...
    'stop_emergency_need': [
      {
        ...
        triggerPhrases: ["need to stop", "pull over", "emergency", "contaminate"],
  ...
  private static dialogueVariations: Record<string, string[][]> = {
    'nervous': [
      ["Please don't leave me alone", "I keep hearing things", "Something's following me"],
      ["The shadows look wrong tonight", "Do you feel that too?", "We're not alone, are we?"],
      ["I shouldn't have come out tonight", "They warned me about this", "It's too quiet"]
    ],
    
    'deceptive': [
      ["Everything's fine, just drive", "Nothing to worry about", "I'm perfectly normal"],
      ["Trust me, I know what I'm doing", "The others were wrong about me", "You seem very... perceptive"],
      ["What a lovely night for a drive", "I do this route often", "Such interesting conversations we could have"]
    ],
    
    'desperate': [
      ["You have to help me", "They'll find me soon", "Time is running out"],
      ["Please believe me", "I'm not crazy, I swear", "You're my only hope"],
      ["Don't let them take me back", "I can't go through that again", "Please, I'm begging you"]
    ]
  };
   ...
   */
  static generatePassengerVariation(basePassenger: Passenger, playerExperience: number = 0): Passenger {
    const variation = this.selectBehaviorPattern(basePassenger, playerExperience);
    const variationId = Math.random().toString(36).substr(2, 9);
    
    return {
      ...basePassenger,
      id: basePassenger.id + (Math.random() * 1000), // Unique instance ID
      tells: this.generateVariationTells(basePassenger, variation, playerExperience),
      dialogue: this.generateVariationDialogue(basePassenger, variation),
      stressLevel: this.adjustStressLevel(basePassenger.stressLevel || 0.5, variation, playerExperience),
      deceptionLevel: this.adjustDeceptionLevel(basePassenger.deceptionLevel || 0, variation, playerExperience),
      trustRequired: this.adjustTrustRequired(basePassenger.trustRequired || 0.5, variation, playerExperience),
   ...
   */
  private static selectBehaviorPattern(passenger: Passenger, playerExperience: number): PassengerVariation['behaviorPattern'] {
    ...
    const patterns: PassengerVariation['behaviorPattern'][] = ['nervous', 'desperate', 'calm'];
  ...
  private static generateVariationTells(
    ...
    for (const exceptionId of passenger.guidelineExceptions) {
      ...
      const numTells = behaviorPattern === 'deceptive' ? 1 : Math.floor(Math.random() * 2) + 1;
      const selectedVariations = this.shuffleArray([...variations]).slice(0, numTells);
      
      for (const variation of selectedVariations) {
        const description = variation.descriptions[Math.floor(Math.random() * variation.descriptions.length)];
        const triggerPhrase = variation.triggerPhrases.length > 0 
          ? variation.triggerPhrases[Math.floor(Math.random() * variation.triggerPhrases.length)]
          : undefined;
        
        // Adjust reliability based on experience and behavior
        const baseReliability = variation.reliabilityRange[0] + 
          Math.random() * (variation.reliabilityRange[1] - variation.reliabilityRange[0]);
        ...
        variationTells.push({
          ...
          audioCue: variation.baseType === 'verbal' ? `${behaviorPattern}_speech` : undefined
        });
      }
    }
   ...
   */
  private static generateVariationDialogue(passenger: Passenger, behaviorPattern: PassengerVariation['behaviorPattern']): string[] {
    ...
    const selectedPatternSet = patternDialogue[Math.floor(Math.random() * patternDialogue.length)];
    const mixedDialogue = [...baseDialogue];
    
    // Replace 1-2 base dialogue lines with pattern-specific ones
    const numReplacements = Math.min(selectedPatternSet.length, Math.floor(Math.random() * 2) + 1);
   ...
   */
  private static adjustStressLevel(baseStress: number, behaviorPattern: PassengerVariation['behaviorPattern'], playerExperience: number): number {
   ...
   */
  private static adjustDeceptionLevel(baseDeception: number, behaviorPattern: PassengerVariation['behaviorPattern'], playerExperience: number): number {
   ...
   */
  static generatePassengerVariations(basePassenger: Passenger, count: number = 3, playerExperience: number = 0): Passenger[] {
    const variations: Passenger[] = [];
    
    for (let i = 0; i < count; i++) {
      variations.push(this.generatePassengerVariation(basePassenger, playerExperience + i * 10));
    }
