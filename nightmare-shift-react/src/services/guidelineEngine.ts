...
export class GuidelineEngine {
   ...
   */
  static analyzePassenger(passenger: Passenger, gameState: GameState, guidelines: Guideline[]): DetectedTell[] {
  ...
  private static passengerMatchesException(passenger: Passenger, exception: GuidelineException): boolean {
    ...
    return exception.conditions.every(condition => {
      switch (condition.type) {
        ...
        case 'passenger_behavior':
          return passenger.stressLevel !== undefined && 
  ...
  private static compareValues(actual: number, expected: string | number, operator?: string): boolean {
  ...
  private static findActiveException(
    ...
    for (const exception of guideline.exceptions) {
      if (this.passengerMatchesException(passenger, exception) &&
  ...
  private static evaluateDecisionCorrectness(consequences: GuidelineConsequence[]): boolean {
    return consequences.some(c => c.type === 'survival' || (c.type === 'reputation' && c.value > 0));
  }
}