...
final class GameSaveRepository implements GameSaveStore
{
    ...
    public function save(int $userId, array $gameState, string $version = '1.0.0'): GameSave
    {
        ...
        $stmt = $this->db->prepare('INSERT INTO game_saves (user_id, game_state, version, created_at, updated_at)
    ...
    {
        $stmt = $this->db->prepare('UPDATE game_saves SET updated_at = CURRENT_TIMESTAMP WHERE user_id = :user_id');
        $stmt->execute(['user_id' => $userId]);
    }
