...
final class BackstoryRepository
{
    ...
    {
        $stmt = $this->db->prepare('SELECT passenger_id FROM backstory_progress WHERE user_id = :user_id');
    ...
    {
        $stmt = $this->db->prepare('INSERT IGNORE INTO backstory_progress (user_id, passenger_id, unlocked_at)
    ...
    {
        $stmt = $this->db->prepare('SELECT COUNT(*) FROM backstory_progress
